...
class PingThread(QRunnable):
    ...
    def ping(self):
        ...
        if self.system == "Windows":
            ...
            if "TTL" in stdout:
                result = [time] + [int(x.strip("ms")) for x in re.findall("\d+ms", stdout_lines[-1])]
            elif "Destination host unreachable" in stdout_lines[1]:
                result = [stdout_lines[1].strip(".")]
            else:
                result = [stdout_lines[2].strip(".")]
            ...
            if "round-trip" in stdout:
                ...
                result = [f"Reply from {ip}: Destination host unreachable"]
            elif "Destination Host Unreachable" in stdout_lines[2]:
                ip = re.findall("\d+.\d+.\d+.\d+", stdout_lines[2])[0]
                result = [f"Reply from {ip}: Destination host unreachable"]
            elif "Request timeout" in stdout_lines[1]:
            ...
            if "ttl" in stdout:
                ...
                ip = re.findall("\d+.\d+.\d+.\d+", stdout_lines[1])[0]
                result = [f"Reply from {ip}: Destination host unreachable"]
            elif "ping statistics" in stdout_lines[1]:
                result = ["Request timed out"]
            else:
                result = [stdout_lines[1]]
